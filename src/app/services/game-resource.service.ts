    ...
    public getResourcePack(packId:string):Observable<ResourcePack> {
        return from(new Promise<ResourcePack>(async (resolve, reject) => {
            try {
                ...
                if(existing) {
                    console.debug(`Successfully loaded Resource Pack '${existing.packMetadata.name} - ${existing.packMetadata.version}' from browser cache.`);
                    resolve(existing);
                    return;
                }
                console.debug(`Unable to find Resource Pack '${metadata.name} - ${metadata.version}' in browser cache.`);
