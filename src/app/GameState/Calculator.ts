...
export function unitQualifiesForEffect(
    ...
    let unitTypeRequired = effect.unitTypeRequired != null && effect.unitTypeRequired.length > 0;
    let unitTypeMatches = unitTypeRequired && (effect.unitTypeRequired?.some(name => name === unitType.name) ?? false);
    let terrainTypeRequired = effect.terrainRequired != null && effect.terrainRequired.length > 0;
    let terrainTypeMatches = terrainTypeRequired && (effect.terrainRequired?.some(terrain => terrain === terrainName) ?? false);
    let classificationRequired = effect.classificationRequired != null && effect.classificationRequired.length > 0;
    let classificationMatches = classificationRequired && (effect.classificationRequired?.every(classification => {
        if(classification[0] === '!') {
