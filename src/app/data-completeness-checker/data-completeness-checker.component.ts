...
export class DataCompletenessCheckerComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.gameDataService.getUnitTypes().subscribe({next: list => {
      this.unitTypes = list;
    ...
    this.gameDataService.getTerrainTypes().subscribe({next: list => this.terrainTypes = list});
    this.gameDataService.getWeaponTypes().subscribe({next: list => this.weaponTypes = list});
    this.gameDataService.getCommanderTypes().subscribe({next: list => this.commanderTypes = list});
    this.gameDataService.getMovementClasses().subscribe({next: list => this.movementClasses = list});

    this.gameDataService.getPassiveUnitEffects().subscribe({next: list => this.passiveUnitEffects = list});
    this.gameDataService.getPassiveTerrainEffects().subscribe({next: list => this.passiveTerrainEffects = list});
    this.gameDataService.getPassiveGlobalEffects().subscribe({next: list => this.passiveGlobalEffects = list});
    this.gameDataService.getActiveUnitEffects().subscribe({next: list => this.activeUnitEffects = list});
    this.gameDataService.getActiveTerrainEffects().subscribe({next: list => this.activeTerrainEffects = list});
    this.gameDataService.getActiveGlobalEffects().subscribe({next: list => this.activeGlobalEffects = list});
  }
